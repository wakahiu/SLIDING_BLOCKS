	...
	public static boolean search(){
		...
		if(currentMove==null){
			...
			while(myTrie!=nextNode.getParent()){
				...
				if(!myBoard.isOK()){
					throw new IllegalStateException("Undoing moves and moving into another set of siblings caused an illegal board state");
				}
				myTrie = myTrie.getParent();
			}
			...
			if(!myBoard.isOK()){
				throw new IllegalStateException("The current moved caused an illegal board state");
			}
			...
			if(deadEnds.contains(myBoard.getTray())||alreadySeen.contains(myBoard.getTray())){
				myBoard.move(currentMove.getMoveLength(),currentMove.getMoveWidth(),
						...
						currentMove.getNumMatchGoal());
				if(!myBoard.isOK()){
					throw new IllegalStateException("Undoing a dead end of a move already seen caused an illegal board state");
				}
				if (!fringe.isEmpty()){
					myTrie=fringe.pop();
				} else {
